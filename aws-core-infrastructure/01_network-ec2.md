...
# 2.サブネットを作成
サブネットは、VPCの中をさらに小さく分けたネットワーク区画
...
# 3. インターネットゲートウェイの作成
インターネットゲートウェイは、VPCとインターネットをつなぐ出入口
...
## TeraTermで接続
TeraTermは管理者権限で起動<br>
my-EC2-Keyを実行する際に権限が必要なため<br>
my-EC2-Keyの権限は広すぎたため実行できず、権限を管理者だけに変更している<br>
